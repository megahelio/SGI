...
// Definimos parámetros para el cocodrilo
...
void DibujaCocodrilo(void) {
	// Cuerpo
	glPushMatrix();
	glTranslatef(0.0, 0.0, 0.0);  // Posición del cuerpo
	...
	// Cabeza
	glPushMatrix();
	glTranslatef(TAM_CUERPO, 0.0, 0.0);  // Posición de la cabeza
	glScalef(TAM_CABEZA, 0.5, 0.5);  // Escala de la cabeza
	igWireRulo(10, 8);  // Crear cabeza
	glPopMatrix();

	// Ojos (2 esferas pequeñas)
	glPushMatrix();
	...
	// Cola
	glPushMatrix();
	glTranslatef(-TAM_COLA, 0.0, 0.0);  // Posición de la cola
